...
**"Why Next.js?"**
Single repo covers frontend and API routes. One-click Vercel deployment. No separate server to maintain.

**"Why no database?"**
The anonymity model discourages persistent storage of evidence. A prototype does not need it. Production would use encrypted storage with strict retention policies.

**"How does provenance work?"**
Two layers: (1) server timestamp at intake — the source cannot alter when the file entered the system; (2) cross-document coherence — fabricating five mutually consistent documents is extremely hard. Claude checks consistency of dates, names, numbers, and institutional references across the full package.

**"What would you add with more time?"**
Audio transcription via Whisper, proper PDF parsing, a certificate lookup page (read-only, by ID), and anchoring certificate hashes to a public timestamp service (e.g. OpenTimestamps) for independent verification.

**"What did you cut?"**
User auth, database, blockchain anchoring, audio transcription. Each was cut because setup time exceeded the value it added to a 2-hour prototype.

**"How does the AI workflow work?"**
Files are hashed, text is extracted, then sent to Claude in a single structured prompt. Claude returns JSON. The system prompt instructs it to score five dimensions independently, then produce the certificate output. No streaming — the full response is needed before the certificate is assembled.
